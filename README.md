...
#### 8.5.1 操作日志记录中间件
系统包含一个操作日志记录中间件，用于自动记录所有非查询接口的操作。
该中间件会记录POST、PUT、PATCH、DELETE等操作，并存储到操作日志数据库表中。

**特性**:
- 自动记录操作日志
- 支持白名单配置
- 自动识别操作用户
- 详细信息记录（请求参数、响应结果、操作耗时等）

更多详细信息请查看 [操作日志中间件文档](./docs/operation_log_middleware.md)。
...
### 2025-10-17
- 完善领域模型定义，为所有模型添加 `app_label = 'domain'` 配置
- 添加自定义测试运行器，优化测试配置
- 更新项目根目录的 `__init__.py` 文件，确保项目根目录在Python路径中
- 配置Django测试设置，指定测试运行器和测试发现根目录
- 优化模型文件结构，完善各模型的字段定义和文档注释
- 移除README中过时的扩展建议章节
